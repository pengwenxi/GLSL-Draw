...
-(void)deleteRenderAndFrameBuffer{
    /*
    buffer分为framebuffer 和 renderbuffer2个大类。
    其中framebuffer 相当于renderbuffer的管理者。
    framebufferobject即称FBO。
    renderbuffer则又可分为3类。colorBuffer、depthBuffer、stencilBuffer。
    */
...
-(void)setupRenderBuffer{
    ...
    //4、将可绘制对象CAEAGLLayer的存储绑定到OpenGL ES RenderBuffer对象
...
-(void)renderLayer{
    ...
    if(linkStatus == GL_FALSE){
        ...
        NSLog(@"program error : %@",messageStr);
        return;
    }
    ...
    //(3)将attriBuffer绑定到GL_ARRAY_BUFFER标识符上
